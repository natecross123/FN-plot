...
import fnplot.sys.UnboundVarException;
...
public class Environment<T> {
    ...
    HashMap<String, Closure<T>> funDictionary;
    ...
    /**
     * Bind the given function to the given name.
     */ 
    public void putFn(String fnName, Closure<T> function) {
	funDictionary.put(fnName, function);
    }

    /**
     * Lookup and return the function bound to the given name.
     */ 
    public Closure<T> getFn(String name) throws UnboundVarException {
	Closure<T> result = funDictionary.get(name);
	if (result == null) {
	    if (parent != null) {
